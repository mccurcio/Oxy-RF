...
import_polypeptide_line <- function(FILE_2_RUN) {
    ...
    for(i in 1:n) {
        line_2_process <- scan(file = con, 
                               ...
                               quiet = T, 
                               text)
        linear_pp_to_percent_aa(line_2_process) # Process polypeptides to paa.
    }
}
```
...
```{r}
linear_pp_to_percent_aa <- function(line_2_process) {
    # Initialize column naming conventions
    aa_vector = as.vector(matrix(0, ncol = 23))
    code_key <- c("Ctrl", "Ery", "Hcy", "Hgb", "Hhe", "Lgb", "Mgb")
    amino_acids = t(c("G", "P", "A", "V", "L", "I", "M", "C", "F", "Y",
                      "W", "H", "K", "R", "Q", "N", "E", "D", "S", "T"))
    # Loop over 7 Code-Keys to apply "Class" and "id"
    for (i in code_key) {
        # For this test-harness file each 500 lines in a 'Class'
        for (j in 1:500) {
            # First column is protein 'Class'
            aa_vector[1] = i
            # Second column is 'id'
            aa_vector[2] = paste(code_key, j, sep = "-")
            # Third column is 'TotalAA'
            total_aa = nchar(line_2_process,
                             type = "chars",
                             keepNA = FALSE)
            aa_vector[3] = total_aa
            # Columns 4:23 - Calculate percent AA per line
            for (k in 4:23) {
                aa_vector[k] = str_count(line_2_process[1, row], k-4) / total_aa
            }
            aa_nums = t(aa_nums)
        }
        }
